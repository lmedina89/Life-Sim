...
function freshState(name='Player',gender='Male',birthplace='Columbia, SC',background='Balanced',difficulty='Normal'){
  const st={version:D.version,saveSchemaVersion:D.saveSchemaVersion||1,savedAt:null,name,gender,birthplace,location:birthplace,background,difficulty,age:0,cash:0,savings:0,debt:0,credit:null,investments:0,
    ...
    structuredHistory:[],milestones:[],hobbies:{},activityCounts:{},philanthropyTotal:0,travelCount:0,hobbyEarnings:0,exPartners:[],lastAnnualSummary:null,
...
function normalize(raw){
  ...
  m.hobbies=m.hobbies&&typeof m.hobbies==='object'?m.hobbies:{};m.activityCounts=m.activityCounts&&typeof m.activityCounts==='object'?m.activityCounts:{};m.philanthropyTotal=Number(m.philanthropyTotal||0);m.travelCount=Number(m.travelCount||0);m.hobbyEarnings=Number(m.hobbyEarnings||0);
...
function requirementMissing(req){
  ...
  switch(req.type){
    ...
    case'inventoryType':return s.inventory.some(i=>i.type===v)?null:(label||`Own ${v}`);
...
function render(){
  ...
  $('childrenList').innerHTML=s.family.children.map(personRow).join('')||'<p class="muted">No children.</p>';$('friendsList').innerHTML=s.friends.map(personRow).join('')||'<p class="muted">No close friends.</p>';renderFamilyTree();renderHobbies();renderLifeRecord();renderAnnualRecap();bindDataDrivenContent();
...
function applySpecialEffects(def={}){
  ...
  if(def.hobbyGain)for(const[k,v]of Object.entries(def.hobbyGain)){const before=s.hobbies[k]||0;s.hobbies[k]=clamp(before+v);const bt=hobbyTier(before),at=hobbyTier(s.hobbies[k]);if(at.min>bt.min){recordLife('hobbyTier',`${D.hobbies?.[k]?.label||k} reached ${at.name}`,{hobby:k,proficiency:s.hobbies[k]});log(`Age ${s.age}: ${D.hobbies?.[k]?.label||k} reached ${at.name} proficiency.`)}}
  ...
  if(def.inventoryCondition)s.inventory.forEach(i=>i.condition=clamp(i.condition+def.inventoryCondition));
}
function runDefinedAction(def,options={}){if(!def)return false;const miss=missingRequirements(def.requirements||[]);if(miss.length){if(options.messageId)msg(options.messageId,`Locked: ${miss.join(' • ')}.`);else toast(`Locked: ${miss.join(' • ')}`);return false}if(def.cost&&s.cash<def.cost){const text=`Need ${money(def.cost)}.`;if(options.messageId)msg(options.messageId,text);else toast(text);return false}const key=def.actionKey||def.id;if(def.ap&&!actionCost(key,def.ap))return false;if(def.cost)s.cash-=def.cost;applyEffects(def.effects||{},{actionKey:key,scaledKeys:def.scaled||[]});applySpecialEffects(def);let resultText=def.narrative||'';if(def.chanceBase!==undefined){const hobby=Number(s.hobbies?.[def.hobby]||0),chance=Math.max(.05,Math.min(.95,Number(def.chanceBase||0)+hobby/500+s.hidden.luck/1000));const success=Math.random()<chance,outcome=success?(def.successEffects||{}):(def.failureEffects||{});applyEffects(outcome);applySpecialEffects(success?(def.successSpecial||{}):(def.failureSpecial||{}));if(success&&Number(outcome.cash||0)>0)s.hobbyEarnings+=Number(outcome.cash||0);resultText=success?(def.successText||'It went well.'):(def.failureText||'It did not work out this time.');toast(resultText)}if(def.bossQuality!==undefined){const gain=def.bossQualityScaled?scaled(def.bossQuality,key):def.bossQuality;s.bossQuality=clamp((s.bossQuality??50)+gain)}s.activityCounts[def.id||key]=(s.activityCounts[def.id||key]||0)+1;if(def.narrative)log(def.narrative);if(def.recordTitle)recordLife(def.recordType||'activity',def.recordTitle,{activity:def.id});if(def.toast)toast(def.toast);if(def.ap&&(s.actionUse[key]||0)>1&&!def.toast&&def.chanceBase===undefined)toast('Repeated action: reduced benefit');return true}
function chooseEvent(c,e){if(c.cost&&s.cash<c.cost){toast(`Need ${money(c.cost)}.`);return}if(c.ap&&!actionCost(`event:${e.id}`,c.ap))return;if(c.cost)s.cash-=c.cost;let outcome=c.effect||{};if(c.chance){let p=.5;if(c.chance==='scholarship')p=.35+(s.gpa||0)/8+s.intelligence/500+s.hidden.luck/700;if(c.chance==='jobOffer')p=.45+s.social/350+s.performance/500;if(c.chance==='businessDeal')p=.42+s.skills.business/250+(s.business?.reputation||0)/500+s.hidden.luck/900;outcome=Math.random()<p?(c.success||{}):(c.failure||{})}applyEffects(outcome);applySpecialEffects(c);if(c.childBoost){const child=s.family.children.filter(x=>x.alive)[0];if(child){child.skill=clamp((child.skill||50)+c.childBoost);child.quality=clamp(child.quality+4)}}if(c.spawnFriend)makeFriend(true);if(c.delayed)s.pendingConsequences.push({age:s.age+c.delayed.years,text:c.delayed.text,effect:c.delayed.effect});log(`Age ${s.age}: ${e.title} — ${c.result}`);eventQueue.shift();$('eventOverlay').classList.add('hidden');commit();showEvent()}
...
function makeDataButton(def,handler){const b=document.createElement('button');b.type='button';b.textContent=def.label||def.name||def.id;if(def.ap)b.dataset.ap=String(def.ap);b.dataset.contentId=def.id||'';const miss=s?missingRequirements(def.requirements||[]):[];if(miss.length){b.classList.add('locked-content');b.title=`Locked: ${miss.join(' • ')}`}b.onclick=handler;return b}
...
function bindDataDrivenContent(){
  ...
  Object.entries(D.hobbies||{}).forEach(([key,h])=>{if(!hobbies)return;hobbies.appendChild(makeDataButton(h,()=>{const def={...h,actionKey:`hobby:${key}`,scaled:Object.keys(h.effects||{}).filter(k=>(h.effects[k]||0)>0)};const before=s.hobbies[key]||0;if(!runDefinedAction(def))return;const gain=scaled(5,def.actionKey);s.hobbies[key]=clamp(before+gain);const bt=hobbyTier(before),at=hobbyTier(s.hobbies[key]);recordLife('hobby',`Practiced ${h.label}`,{hobby:key,proficiency:s.hobbies[key]});if(at.min>bt.min){recordLife('hobbyTier',`${h.label} reached ${at.name}`,{hobby:key,proficiency:s.hobbies[key]});log(`Age ${s.age}: ${h.label} reached ${at.name} proficiency.`)}commit();msg('hobbyMessage',`${h.label} proficiency is now ${Math.round(s.hobbies[key])}/100 • ${at.name}.`)}))});
  if(opps){let lastHobby='';const visible=(D.hobbyOpportunities||[]).filter(o=>Number(s?.hobbies?.[o.hobby]||0)>0);if(!visible.length){const note=document.createElement('div');note.className='muted';note.textContent='Practice a hobby to reveal its progression opportunities.';opps.appendChild(note)}visible.forEach(o=>{if(o.hobby!==lastHobby){const lab=document.createElement('div');lab.className='activity-group-label';lab.textContent=D.hobbies?.[o.hobby]?.label||o.hobby;opps.appendChild(lab);lastHobby=o.hobby}const b=makeDataButton(o,()=>{if(!runDefinedAction(o,{messageId:'hobbyMessage'}))return;commit();msg('hobbyMessage',`${o.label}: ${money(s.hobbyEarnings||0)} total hobby income.`)});if(s&&missingRequirements(o.requirements||[]).length)b.textContent=`🔒 ${o.label}`;opps.appendChild(b)})}
  if(life){let last='';Object.values(D.lifeActivities||{}).forEach(a=>{if(a.group!==last){const lab=document.createElement('div');lab.className='activity-group-label';lab.textContent=({entertainment:'Entertainment',fitness:'Fitness & Outdoors',travel:'Travel',development:'Personal Development',giving:'Philanthropy'})[a.group]||a.group;life.appendChild(lab);last=a.group}const b=makeDataButton(a,()=>{if(!runDefinedAction(a,{messageId:'lifeActivityMessage'}))return;commit();msg('lifeActivityMessage',`${a.label} complete${a.cost?` • ${money(a.cost)}`:''}.`)});if(s&&missingRequirements(a.requirements||[]).length)b.textContent=`🔒 ${a.label}`;life.appendChild(b)})}
...
function bind(){
  ...
  $('maintainItemsBtn').onclick=()=>{if(!s.inventory.length)return msg('inventoryMessage','You do not own anything to maintain.');if(s.cash<500)return msg('inventoryMessage','Need $500.');s.cash-=500;s.inventory.forEach(i=>i.condition=clamp(i.condition+18));msg('inventoryMessage','Owned items maintained.');commit()};$('buyItemBtn').onclick=()=>{const key=$('itemSelect').value,it=D.inventoryCatalog[key];if(s.cash<it.price)return msg('inventoryMessage',`Need ${money(it.price)}.`);s.cash-=it.price;s.inventory.push({id:uid(),key,name:it.name,type:it.type,condition:100,value:Math.round(it.price*.82),conditionLoss:it.conditionLoss,annualCost:it.annualCost||0,prestige:it.prestige||0});if(it.prestige)stat('happiness',Math.min(4,Math.ceil(it.prestige/3)));recordLife('purchase',`Bought ${it.name}`,{itemKey:key,price:it.price});log(`Age ${s.age}: Bought ${it.name}.`);commit()};
  ...
  $('partnerVacationBtn').onclick=()=>{if(!s.partner)return msg('relationshipMessage','You need a partner first.');if(s.cash<2500)return msg('relationshipMessage','Need $2,500.');if(!actionCost('partnerVacation',2))return;s.cash-=2500;s.partner.quality=clamp(s.partner.quality+10);stat('happiness',7);stat('stress',-6);s.travelCount=(s.travelCount||0)+1;log(`Age ${s.age}: I took a vacation with ${s.partner.name}.`);commit()};
  $('partnerSupportBtn').onclick=()=>{if(!s.partner)return msg('relationshipMessage','You need a partner first.');if(!actionCost('partnerSupport',1))return;s.partner.quality=clamp(s.partner.quality+6);stat('communication',2);stat('karma',2);log(`I supported ${s.partner.name} through something important.`);commit()};
  $('friendHangoutBtn').onclick=()=>{const friends=s.friends.filter(f=>f.alive);if(!friends.length)return msg('friendMessage','You need a living friend first.');if(!actionCost('friendHangout',1))return;friends.forEach(f=>f.quality=clamp(f.quality+4));stat('happiness',4);stat('social',2);log('I spent time with friends.');commit()};
  $('friendGiftBtn').onclick=()=>{const friends=s.friends.filter(f=>f.alive);if(!friends.length)return msg('friendMessage','You need a living friend first.');if(s.cash<150)return msg('friendMessage','Need $150.');if(!actionCost('friendGift',1))return;s.cash-=150;friends[0].quality=clamp(friends[0].quality+7);stat('happiness',2);log(`I gave ${friends[0].name} a gift.`);commit()};
  $('friendHelpBtn').onclick=()=>{const friends=s.friends.filter(f=>f.alive);if(!friends.length)return msg('friendMessage','You need a living friend first.');if(s.cash<500)return msg('friendMessage','Need $500.');if(!actionCost('friendHelp',1))return;s.cash-=500;friends[0].quality=clamp(friends[0].quality+9);stat('karma',4);log(`I helped ${friends[0].name} when they needed it.`);commit()};
  $('childTimeBtn').onclick=()=>{const kids=s.family.children.filter(c=>c.alive);if(!kids.length)return msg('childSupportMessage','You do not have a living child.');if(!actionCost('childTime',1))return;kids.forEach(k=>k.quality=clamp(k.quality+5));stat('happiness',4);log('I spent one-on-one time with my children.');commit()};
  $('childGiftBtn').onclick=()=>{const kids=s.family.children.filter(c=>c.alive);if(!kids.length)return msg('childSupportMessage','You do not have a living child.');if(s.cash<250)return msg('childSupportMessage','Need $250.');if(!actionCost('childGift',1))return;s.cash-=250;kids[0].quality=clamp(kids[0].quality+6);stat('happiness',2);log(`I bought ${kids[0].name} a gift.`);commit()};
  $('childMoneyBtn').onclick=()=>{const adults=s.family.children.filter(c=>c.alive&&c.age>=18);if(!adults.length)return msg('childSupportMessage','You do not have an adult child to support.');if(s.cash<5000)return msg('childSupportMessage','Need $5,000.');s.cash-=5000;adults[0].quality=clamp(adults[0].quality+7);stat('karma',4);log(`Age ${s.age}: I gave ${adults[0].name} $5,000 to help them out.`);commit()};
