# Life Sim v0.8.0.2 — Education Requirement Hotfix
...
## v0.8.0.1 — Career Consistency Hotfix
- Enforces each job's declared education requirement in the actual qualification engine.
- Corrects backwards promotion ages and salary-floor regressions.
- Preserves mandatory professional credentials through careers where the credential remains logically required.
- Normalizes obvious experience, reputation, fitness and skill requirement regressions.
- Makes the Science management ladder a true continuation of Principal Scientist, including Graduate/PhD path, Science skill, Business, reputation and Science-experience requirements.
- Adds career-consistency validation so future job data is automatically checked for broken promotion links, cycles, education mismatches, backwards ages and salary-floor regressions.
- Save schema remains 7; no save reset is required.


## v0.8.0.2 — Education Requirement Hotfix
- Career education checks now use the character's full completed education history rather than only the latest/current degree state.
- `graduatePathIn` recognizes any completed qualifying graduate path, so a later MD/MBA/etc. does not erase an earlier PhD/MS qualification.
- `majorIn` recognizes any completed college major retained in education history.
- `educationMin` uses the highest completed education level and cannot be downgraded by later schooling.
- Legacy saves without detailed graduate history retain a narrow compatibility fallback.
- Save schema remains 7; no save reset is required.
